...
const SECTORS = ["M&A", "ECM", "DCM", "FIG", "TMT", "Healthcare", "Industrials", "Natural Resources", "Real Estate"];
  ...
  return (
    <div className="top-strip">
      <div className="top-strip-inner">
        <span className="strip-item"><span className="dot"></span>Agent Online</span>
        <span className="sep">|</span>
        <span className="strip-item">HKT {hh}:{mm}</span>
        <span className="sep">|</span>
        <span className="strip-item">HKEX <span className="ticker">26,418.30 +0.42%</span></span>
        <span className="sep">|</span>
        <span className="strip-item">USD/HKD <span className="ticker">7.8104</span></span>
        <div className="right">
      ...
      <header className="masthead">
        <div className="masthead-inner">
          ...
          <div className="masthead-title">
            ...
            <h1>The <span className="accent">IBD</span> Compass</h1>
    ...
    <section className="hero">
      ...
      <div className="hero-stats">
        <div className="stat-cell">
          ...
          <div className="delta"><span className="up">↑</span> Fresh postings ·  5:47 AM scan</div>
        </div>
        <div className="stat-cell">
...
function FilterBar({ activeSector, setActiveSector, sectorCounts, query, setQuery, activeStatus, setActiveStatus, statusCounts }) {
  return (
    <div className="filter-bar">
      <span className="label-tiny">Sector</span>
      <button className={`chip ${activeSector === "ALL" ? "active" : ""}`} onClick={() => setActiveSector("ALL")}>
      ...
      <input className="search-input" placeholder="Search company or title…" value={query} onChange={e => setQuery(e.target.value)} />

      <div className="status-toggle">
        <button className={activeStatus === "all" ? "active" : ""} onClick={() => setActiveStatus("all")}>All <span className="count">({statusCounts.all})</span></button>
        <button className={activeStatus === "interested" ? "active" : ""} onClick={() => setActiveStatus("interested")}>★ {statusCounts.interested}</button>
        <button className={activeStatus === "applied" ? "active" : ""} onClick={() => setActiveStatus("applied")}>✓ {statusCounts.applied}</button>
        <button className={activeStatus === "hidden" ? "active" : ""} onClick={() => setActiveStatus("hidden")}>Hide {statusCounts.ignored}</button>
      </div>
    </div>
  );
}
...
function Tracker({ statuses, jobs }) {
  const byStage = useMemo(() => {
    ...
    Object.entries(statuses).forEach(([id, st]) => { if (map[st]) map[st].push(id); });
    return map;
  }, [statuses]);

  const jobsById = useMemo(() => Object.fromEntries(jobs.map(j => [j.id, j])), [jobs]);
  const totalActive = window.PIPELINE_KEYS.reduce((n, k) => n + byStage[k].length, 0);
  const offerCount = byStage.offer.length;

  return (
    <div className="tracker-block">
      <div className="tracker-head">
        <div className="title">Application <span className="accent">Tracker</span></div>
        <div style={{fontFamily:"var(--f-mono)", fontSize:10, letterSpacing:".15em", color:"var(--gold-bright)"}}>
      ...
      <div className="tracker-stages">
        {STAGES.map((s) => {
          ...
          return (
            <div className={`stage ${hasItems ? "has-items" : ""} ${s.key === "offer" && hasItems ? "is-offer" : ""}`} key={s.key}>
              <div style={{flex: 1}}>
                <div className="stage-left">
                  <span className="step">{s.step}</span>
                  <span className="name">{s.label} <span style={{color:"var(--ink-mute)", fontStyle:"italic"}}>· {s.labelEn}</span></span>
                </div>
                {hasItems && (
                  <div className="stage-companies">
                    {ids.slice(0, 8).map(id => {
                      const j = jobsById[id];
                      if (!j) return null;
                      return <span key={id} className="stage-comp-pip" title={`${j.company} — ${j.title}`}>{j.companyShort}</span>;
                    })}
  ...
  return (
    <div className="agent-box">
      ...
      <div className="agent-log">
        <div className="line"><span className="time">05:42</span><span>Initialising scan across 22 banks…</span></div>
        <div className="line"><span className="time">05:43</span><span>Crawling <span className="num">22</span> career pages</span></div>
        <div className="line"><span className="time">05:45</span><span>Filtering <span className="num">VP · HK · IBD</span></span></div>
        <div className="line"><span className="time">05:47</span><span className="ok">✓ Found {stats.newToday} new role{stats.newToday !== 1 ? "s" : ""}</span></div>
        <div className="line"><span className="time">05:47</span><span className="ok">✓ Brief delivered to 屁宝</span></div>
        <div className="line"><span className="time">—</span><span style={{color:"var(--ink-mute)"}}>Next scan: tomorrow 05:30 HKT</span></div>
      </div>
    </div>
  );
}
...
function App() {
  ...
  const statusCounts = useMemo(() => {
    ...
    const applied = Object.values(statuses).filter(s => ["applied","first","final","offer"].includes(s)).length;
  ...
  const filtered = useMemo(() => {
    return jobs.filter(j => {
      ...
      if (activeStatus === "applied" && !["applied","first","final","offer"].includes(st)) return false;
      if (activeStatus === "hidden" && st !== "ignored") return false;
      if (activeSector !== "ALL" && j.sector !== activeSector) return false;
      if (query) {
        const q = query.toLowerCase();
        if (!(j.company.toLowerCase().includes(q) || j.title.toLowerCase().includes(q) || (j.subSector||"").toLowerCase().includes(q))) return false;
      }
      return true;
  ...
  const stats = { ...window.JOB_STATS, tracked: trackedCount };
  ...
  return (
    <>
      ...
      <FilterBar
        activeSector={activeSector} setActiveSector={setActiveSector} sectorCounts={sectorCounts}
        query={query} setQuery={setQuery}
        activeStatus={activeStatus} setActiveStatus={setActiveStatus} statusCounts={statusCounts}
      />

      <main className="main">
        <section>
          <div className="section-head">
            <h3>The <span className="ital">Listings</span></h3>
            <div className="meta">{filtered.length} of {jobs.length} roles · sorted by recency</div>
          </div>
          <div className="jobs-list">
            {filtered.map(job => (
              <window.JobCard key={job.id} job={job} status={statuses[job.id]} onStatus={onStatus} />
            ))}
            {filtered.length === 0 && (
              <div style={{padding: "60px 20px", textAlign:"center", color:"var(--ink-mute)", fontFamily:"var(--f-serif)", fontStyle:"italic", fontSize: 18}}>
                没有匹配的岗位。换个筛选试试。
              </div>
            )}
          </div>
      ...
      <footer className="footer">
        ...
        <div><span className="heart">♥</span> Made with love · Scans daily at 05:30 HKT</div>
      </footer>
    </>
  );
}
